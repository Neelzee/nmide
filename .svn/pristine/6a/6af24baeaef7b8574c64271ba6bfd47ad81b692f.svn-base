        ...
        let ui = Html::Div()
            ...
            .adopt(
                Html::Form()
                    ...
                    .adopt(
                        Html::Div()
                            .adopt(Html::Label().set_text("Module path").add_attr(Attr::Custom(
                                "for".to_string(),
    ...
    async fn handler(&self, event: Event, core: Box<dyn Core>) {
        match (event.event_name(), event.args()) {
            ...
            ("add-module", Some(val)) => {
                ...
                let module_path = args.get("module_path").unwrap().str().unwrap();
                fs::copy(module_path, core.appdir().await).unwrap();
            }
            _ => (),
        }
    }
