...
Running third-party-code can dangerous. If this code is not validate or doesn't
come from a trusted source, it could be an attack vector. Luckily, Tauri does
some of this work for us, but even if we have validated that a module is wanted,
there is still the remaining use that has been the reason for 100\% of all
CVE's, human error.
...
In the \gls{jsms}, managing of modules can lead to is exceptions being thrown.
...
The listing \ref{lst:inst}, it is clear how the \textbf{Instruction}-set makes the
...
instruction set, remove it, then the modifying instruction is an $NoOp$. This
optimization can be found in appendix \ref{app:a}.
...
\begin{center}
  \lstinputlisting
    ...
    , firstline=8
    , lastline=12
    ]{./core/src-tauri/core-std-lib/src/event.rs}
\end{center}
...
dependency graph, so that they could see what modules are connected.
