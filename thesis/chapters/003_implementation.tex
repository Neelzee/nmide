...
Python. But it is easier to do \gls{abi} stuff in a lower level language like
Rust. But what does \textit{lower level} language mean? And what is an \gls{abi}?
...
has changed, as C is considered to be a low level language, since one codes
close to how a \gls{cpu} thinks.
...
\todo{Explain low level \cite{cNotLowLevel}}
...
The new module architecture is shown in figure \ref{fig:moduleArchitecture}. A
module is initialized by invoking the \textbf{init} method, which returns a
...
This is equivalent to a microservice architecture, in the sense that a module
can send a request, (Msg), to another module, and later get a response, (Msg).
\todo{Add diagram?}
