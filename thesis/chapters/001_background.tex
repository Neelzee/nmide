...
Assuming we have some type \bold{Log(A)}, where there the type \bold{A}, is the
...
of the \Bold{Log(A)}-type matters. Representing the composition of the \bold
Log(A)}-type as $\odot$, doing, and letting $a, b, c$ be of type \bold{Log A}:
...
\subsection{Magnolia: Magma \to Group}
...
\todo{Fact check, cite and expand}
