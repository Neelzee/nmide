...
installModule(
  {
    ...
    init: async (core: Core): CoreModification => {
      ...
      await core.registerHandler("DependencyViewer", "graph", null)
        .catch(err => console.error("error from module: ", err));
      return new UiBuilder()
        .add(new HtmlBuilder().kind("svg").attrs({ id: "graph" }), null, null)
        .build();
    },
    handler: async (event: Event, __) => {
  ...
  `
        ...
        const data = processData(event.args);
